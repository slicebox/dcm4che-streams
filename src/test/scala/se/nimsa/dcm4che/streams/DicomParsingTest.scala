...
class DicomParsingTest extends FlatSpecLike with Matchers {
  ...
  "DicomParsing" should "parse headers for explicit VR little endian" in {
    ...
    val maybeHeader = DicomParsing.readHeaderExplicitVR(bytes, assumeBigEndian = false)
  ...
  it should "parse headers for explicit VR big endian" in {
    ...
    val maybeHeader = DicomParsing.readHeaderExplicitVR(bytes, assumeBigEndian = true)
    maybeHeader.isDefined shouldBe true
    val (tag, vr, headerLength, length) = maybeHeader.get
    tag shouldEqual PatientName
    vr shouldEqual VR.PN
  ...
  it should "parse a UID data element for explicit VR little endian" in {
    ...
    val msuid = DicomParsing.parseUIDAttribute(bytes, explicitVR = true, assumeBigEndian = false)
  ...
  it should "parse a UID data element for implicit VR little endian" in {
    ...
    val msuid = DicomParsing.parseUIDAttribute(bytes, explicitVR = false, assumeBigEndian = false)
  ...
  it should "transform a ByteString of size 2 to the correct short representation" in {
    val s1 = DicomParsing.bytesToShort(ByteString(0xA, 0x5), bigEndian = true)
    s1 shouldBe a[java.lang.Short] // runtime type is java native short
    s1 shouldBe 0xA05
    val s2 = DicomParsing.bytesToShort(ByteString(0xA, 0x5), bigEndian = false)
    s2 shouldBe a[java.lang.Short]
    s2 shouldBe 0x50A
  }

  it should "transform a ByteString of size 4 to the correct integer representation" in {
    val s1 = DicomParsing.bytesToInt(ByteString(0xA, 0xB, 0xC, 0xD), bigEndian = true)
    s1 shouldBe a[java.lang.Integer] // runtime type is java native short
    s1 shouldBe 0x0A0B0C0D
    val s2 = DicomParsing.bytesToInt(ByteString(0xA, 0xB, 0xC, 0xD), bigEndian = false)
  ...
  it should "transform a ByteString of size 8 to the correct long representation" in {
    val s1 = DicomParsing.bytesToLong(ByteString(1, 2, 3, 4, 5, 6, 7, 8), bigEndian = true)
    s1 shouldBe a[java.lang.Long]
    s1 shouldBe 0x0102030405060708L
    val s2 = DicomParsing.bytesToLong(ByteString(1, 2, 3, 4, 5, 6, 7, 8), bigEndian = false)
  ...
  it should "transform a ByteString of size 8 to the correct double representation" in {
    ...
    val s1 = DicomParsing.bytesToDouble(beBytes, bigEndian = true)
    s1 shouldBe a[java.lang.Double]
    s1 shouldBe math.Pi
    val s2 = DicomParsing.bytesToDouble(beBytes.reverse, bigEndian = false)
  ...
  it should "transform a ByteString of size 4 to the correct float representation" in {
    ...
    val s1 = DicomParsing.bytesToFloat(beBytes, bigEndian = true)
    s1 shouldBe a[java.lang.Float]
    s1 shouldBe math.Pi.toFloat
    val s2 = DicomParsing.bytesToFloat(beBytes.reverse, bigEndian = false)
  ...
  it should "transform a ByteString of size 2 to the correct unsigned short representation (as an Int)" in {
    val s1 = DicomParsing.bytesToUShort(ByteString(0xFF, 0xFE), bigEndian = true)
    s1 shouldBe a[java.lang.Integer]
    s1 shouldBe 0xFFFE
    val s2 = DicomParsing.bytesToUShort(ByteString(0xFF, 0xFE), bigEndian = false)
    s2 shouldBe a[java.lang.Integer]
    s2 shouldBe 0xFEFF
  }
  ...
  it should "parse a tag number given as two consecutive short numbers (big or little endian)" in {
    ...
    val t1 = DicomParsing.bytesToTag(s1.concat(s2), bigEndian = true)
    t1 shouldBe a[java.lang.Integer]
    t1 shouldBe 0x0A0B0102
    val t2 = DicomParsing.bytesToTag(s1.concat(s2), bigEndian = false)
  ...
  it should "parse two bytes in big-endian order as a VR code" in {
    ...
    DicomParsing.bytesToVR(vr) shouldBe 0xABCD
  }

  it should "decode a short into bytes" in {
    DicomParsing.shortToBytes(0xABCD.toShort, bigEndian = true) shouldBe ByteString(0xAB, 0xCD)
    DicomParsing.shortToBytes(0xABCD.toShort, bigEndian = false) shouldBe ByteString(0xCD, 0xAB)
  }

  it should "decode an integer into bytes" in {
    DicomParsing.intToBytes(0x01020304, bigEndian = true) shouldBe ByteString(1, 2, 3, 4)
    DicomParsing.intToBytes(0x01020304, bigEndian = false) shouldBe ByteString(4, 3, 2, 1)
  }

  it should "not read explicit VR attribute headers with insufficient length" in {
    DicomParsing.readHeaderExplicitVR(ByteString.empty, assumeBigEndian = true) match {
      case Some(_) => fail
      case None =>
    }
    ...
    DicomParsing.readHeaderExplicitVR(tagBytes ++ vrBytes ++ lengthBytes, assumeBigEndian = true) match {
      case Some(_) => fail
      case None =>
    }
  }
  ...
  it should "read explicit VR sequence attribute headers with valid length" in {
    ...
    DicomParsing.readHeaderExplicitVR(tagBytes ++ lengthBytes, assumeBigEndian = true) match {
      case Some((tag, vr, headerLength, valueLength)) =>
  ...
  it should "not read explicit VR sequence attribute headers with undefined length" in {
    ...
    DicomParsing.readHeaderExplicitVR(tagBytes ++ vrBytes, assumeBigEndian = true) match {
      case Some(_) =>
        fail
      case None =>
    }
  ...
  it should "not read implicit VR attribute headers with insufficient length" in {
    DicomParsing.readHeaderImplicitVR(ByteString.empty) match {
      case Some(_) => fail
      case None =>
    }
  }
  ...
  it should "read implicit VR attribute headers with valid length" in {
    ...
    DicomParsing.readHeaderImplicitVR(tagBytes ++ lengthBytes) match {
      case Some((tag, vr, headerLength, valueLength)) =>
        tag shouldBe 0x00080008
        vr shouldBe VR.CS
        headerLength shouldBe 8
